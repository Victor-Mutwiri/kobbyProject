...
function quick_links(element){
  element.children[2].addEventListener('click', ()=>{
    element.firstElementChild.click()
    jQuery('#main2>nav>button')[2].click()
  })
...
function create_project(data){
  ...
  project.lastElementChild.addEventListener('click', (event)=>{
    let b_d = project.lastElementChild.firstElementChild.lastElementChild.lastElementChild
    b_d.style.display = 'block'
    jQuery(b_d).on('mouseleave', (event)=>{
      jQuery(b_d).css('display', 'none')
    })
  })
  quick_links(project)
...
jQuery(document).ready(function(){
      ...
      body: JSON.stringify(project)
       ...
       if (response.ok){
         response.text().then((data)=>{
           project['id'] = data
           _url = "http://localhost:8000/projects/?projectid=" + project['id']
  ...
  jQuery('.createprjj').bind('click', (event)=>{
    jQuery('.bottom-link')[1].click()
  })
})
